...
module.exports = {
  ...
  packageService() {
    ...
    let packagePromises = allFunctions.reduce((previousPromise, functionName) => {
      return previousPromise.then(() => {
        ...
        if (functionObject.package.individually || this.serverless.service.package.individually) {
          return this.buildFunction(functionName).then(() => this.packageFunction(functionName));
        }
    ...
    packagePromises = allLayers.reduce((previousPromise, layerName) => {
